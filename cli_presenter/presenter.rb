class Presenter
  ...
  def call
    @people.each do |person|
      ...
      progress(payments_sum, transaction_tags_sum)
      ngo_report(transaction_tags_sum_per_non_profit, person)
    end
    line
    nil
  end
  ...
  def ngo_report(sum_per_non_profit, person)
    sum_per_non_profit.each do |non_profit, amount|
      puts "VALUE: \e[32m$#{ic(amount).to_s.ljust(8)}\e[0m FROM: #{person.name.ljust(10)} TO: \e[35m#{non_profit.name}\e[0m"
    end
  end


  def progress(total_amount, used_amount)
